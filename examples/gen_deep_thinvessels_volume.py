...
VOL_SZ = (500, 500, 180); VRES = 1; SEED = 42; NT = 200; FOCAL_UM = 45.0
sig = hashlib.sha1(f"deepthinves_{VOL_SZ}_{VRES}_{SEED}".encode()).hexdigest()[:10]
cache = os.path.join(SHARED, f"phase1_deepthinves_{sig}.pkl")
...
run_dir = os.path.join(OUT, f"deepthinves_volume_500um_d180_{_dt.datetime.now():%Y%m%d_%H%M%S}")
os.makedirs(run_dir, exist_ok=True)
meta = dict(tag="deepthinves_volume", phase1_cache=cache, seed=SEED, nt=NT, vres=VRES,
